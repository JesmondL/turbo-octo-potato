...
First part is to process the data with selection of desire data columns (catagories). 
Choose how much of historical data (n_past) wish to remember and use it to 
predict how far into the future (n_future). Larger historical data yields 
higher accuracy. Data needs to be scaled and transformed to the range of 0 to 1
and reduce the SD of 1 and 0 mean with MinMaxScaler fit transform.
...
Second part is create structure of the network.
...
Third part is train the network.

Multiple inputs column increases the learning ability to better gives the prediction
...
def DataPreprocessing(dataset_train, X_train, Y_train):
    ...
    for i in range (n_past, len(dataset_train)):
        # memorized n_past num of past data for prediction, 0 to n_past
        X_train.append(training_set_scaled[i-n_past:i, 0])
        # exclude past data info, RNN will use to predict the incoming n_past to 1258
